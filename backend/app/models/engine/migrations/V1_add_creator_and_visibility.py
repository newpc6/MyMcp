...
def run(db):
    ...
    try:
        ...
        if 'creator_id' not in column_names:
            em_logger.info("添加creator_id字段到mcp_modules表")
            db.execute(text("""
            ...
            # 创建索引
            db.execute(text("""
                CREATE INDEX ix_mcp_modules_creator_id ON mcp_modules (creator_id)
            """))
        else:
            em_logger.info("creator_id字段已存在，跳过")
        ...
        db.commit()
        em_logger.info("成功完成迁移: 添加creator_id和is_public字段")
        return True
